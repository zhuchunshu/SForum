...
func TestGeneratePluginScaffoldNonInteractive(t *testing.T) {
	...
	if manifest.Type != extensions.TypePlugin || manifest.Backend.Entry != "backend/plugin" {
...
func TestGenerateThemeScaffoldNonInteractive(t *testing.T) {
	...
	manifest := readGeneratedManifest(t, target)
	if manifest.Type != extensions.TypeTheme || manifest.Frontend.Layer != "layer" {
...
func readGeneratedManifest(t *testing.T, dir string) extensions.Manifest {
	t.Helper()
	body, err := os.ReadFile(filepath.Join(dir, extensions.ManifestFileName))
	if err != nil {
