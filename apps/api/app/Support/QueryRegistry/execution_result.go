...
func executionCacheTags(plan QueryPlan, filterPlan, providerDigest string) []string {
	...
	for _, tag := range plan.CacheTags {
		digest := sha256.Sum256([]byte(resultCacheSchemaVersion + "\x00invalidation\x00" + tag))
		opaqueTag := hex.EncodeToString(digest[:16])
		// The shared tag deliberately excludes actor, locale, request shape, page,
		// provider, and artifact identity. One semantic mutation can therefore
		// evict every isolated variant that declared the same owned cache tag.
