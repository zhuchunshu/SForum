...
// Registry publishes a complete immutable content graph. Writers validate and
...
func (r *Registry) publish(expected *Artifact, publication Publication) (uint64, error) {
	...
	next, err := buildState(
		current.revision+1, publicationValues(publications), current.tombstoneValues(),
		current.historyValues(), current.safeMode, current,
	)
	if err != nil {
...
// ReplaceAll performs only the first revision-zero graph load. Every later full
// replacement must use ReplaceAllIfRevision with a revision observed before the
// caller builds its candidate graph; otherwise a delayed reconciler could sample
// a fresh revision and replay a stale artifact set over it.
//
// When safeMode is true, non-core publications are filtered from the input
// before validation so corrupt third-party content cannot block Host recovery.
func (r *Registry) ReplaceAll(publications []Publication, safeMode bool) (uint64, error) {
	...
	return r.replaceAllIfRevision(
		current.revision, publications, current.tombstoneValues(), current.historyValues(), safeMode,
	)
}
...
func (r *Registry) ReplaceAllIfRevision(
	...
	current := r.load()
	return r.replaceAllIfRevision(
		expectedRevision, publications, current.tombstoneValues(), current.historyValues(), safeMode,
	)
}

// RestoreIfRevision is the startup/reconciliation path. Host storage supplies
// append-only tombstone and exact-package history rows before any third-party
// declaration becomes active. Missing input rows cannot erase process-local
// history already observed by this Registry.
func (r *Registry) RestoreIfRevision(
	expectedRevision uint64,
	publications []Publication,
	...
	return r.replaceAllIfRevision(expectedRevision, publications, tombstones, history, safeMode)
}
...
func (r *Registry) replaceAllIfRevision(
	...
	if observed.revision != expectedRevision {
		return observed.revision, ErrRevisionConflict
	}
	// 完整图先在锁外构建，锁内只做 revision CAS、exact replay 校验和发布。
	next, err := buildState(0, publications, tombstones, history, safeMode, observed)
	if err != nil {
		return r.load().revision, err
	}
	r.mu.Lock()
	...
	if err := validateCorePublicationTransition(current.publications, next.publications); err != nil {
		return current.revision, err
	}
...
// Snapshot returns a deep-copied inspectable view of the active graph.
...
// Resolve returns one exact content contribution from the active snapshot.
func (r *Registry) Resolve(contentID string) (Contribution, error) {
	if r == nil {
		return Contribution{}, ErrInvalid
	}
	contribution, ok := r.load().content[strings.ToLower(strings.TrimSpace(contentID))]
	if !ok {
...
// List returns content contributions for one frozen kind (or all kinds when
// kind is empty). Ordering is deterministic: kind, then id, then extension.
...
func (r *Registry) String() string {
	state := r.load()
	return fmt.Sprintf("ContentRegistry(revision=%d,digest=%s,safeMode=%t)", state.revision, state.digest, state.safeMode)
}
