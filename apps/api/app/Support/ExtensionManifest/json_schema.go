...
func compiledManifestSchema(fragment string) (*jsonschema.Schema, error) {
	manifestSchemaOnce.Do(func() {
		...
		manifestSchemas = map[string]*jsonschema.Schema{}
		fragments := []string{"", "guards", "schedules", "components", "templates", "assets", "content", "database", "cache", "services", "commands", "adminSurfaces", "queries", "identity", "permissionDefinitions", "media", "navigation", "regions", "dependencies", "lifecycle", "openapi", "packageFiles"}
