...
func compiledManifestSchema(fragment string) (*jsonschema.Schema, error) {
	manifestSchemaOnce.Do(func() {
		...
		manifestSchemas = map[string]*jsonschema.Schema{}
		fragments := []string{"", "guards", "schedules", "components", "templates", "assets", "content", "editor", "database", "cache", "seo", "services", "commands", "adminSurfaces", "queries", "queryResultFilters", "identity", "permissionDefinitions", "notificationTypes", "media", "navigation", "regions", "dependencies", "lifecycle", "openapi", "packageFiles"}
