...
func TestPostgresFrontendTrustStoreCreateCanonicalizesJSONSets(t *testing.T) {
	...
	if got := string(db.queries[0].args[4].([]byte)); got != `["admin.test.fixture","admin.test.secondary"]` {
		t.Fatalf("points JSON is not canonical: %s", got)
	}
	if got := string(db.queries[0].args[5].([]byte)); got != `["primary","secondary"]` {
...
func TestPostgresFrontendTrustStoreFinalizesOnlyPendingGrantsOutsideRelease(t *testing.T) {
	...
	for _, clause := range []string{
		...
		"release_extensions.package_digest = grants.package_digest",
	} {
...
func TestPostgresFrontendTrustStoreRejectsInvalidStoredJSON(t *testing.T) {
	...
	_, err := newPostgresFrontendTrustStore(db).FrontendGrant(context.Background(), grant.ExtensionID, grant.ExtensionVersion, grant.PackageDigest)
...
func frontendGrantRawJSONTestRow(grant FrontendTrustGrant, points []byte, components []byte) pgx.Row {
	return frontendTrustRowFunc(func(dest ...any) error {
		...
		*dest[4].(*int) = grant.APIVersion
		*dest[5].(*[]byte) = points
		*dest[6].(*[]byte) = components
		*dest[7].(*int64) = grant.GrantedByUserID
		*dest[8].(*time.Time) = grant.GrantedAt
		*dest[9].(**time.Time) = grant.RevocationRequestedAt
		*dest[10].(*int64) = grant.RevocationRequestedByUserID
		*dest[11].(**time.Time) = grant.RevokedAt
		*dest[12].(*int64) = grant.RevokedByUserID
		return nil
	})
}
