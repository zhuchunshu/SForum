...
func (a forumNotificationAdapter) NotifyCommentTx(ctx context.Context, tx pgx.Tx, input forum.CommentNotificationInput) error {
	return a.outbox.NotifyCommentTx(ctx, tx, notifications.CommentEvent{CommentID: input.CommentID, TopicID: input.TopicID, ActorUserID: input.ActorUserID, ParentAuthorUserID: input.ParentAuthorUserID, MentionedUsernames: input.MentionedUsernames})
}
