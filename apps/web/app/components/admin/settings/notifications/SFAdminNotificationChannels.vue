...
<template>
  <section class="space-y-4 border-t border-slate-200 pt-5 dark:border-zinc-800" data-testid="admin-notification-channels">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <h2 class="font-semibold">{{ t('admin.notificationSettings.channelsAdmin.title') }}</h2>
        <p class="mt-1 text-sm text-muted">{{ t('admin.notificationSettings.channelsAdmin.description') }}</p>
      </div>
    ...
    <div v-if="pending && channels.length === 0" class="space-y-3" aria-busy="true">
      <SFSkeleton class="h-40 w-full" />
    </div>
    <SFEmptyState v-else-if="channels.length === 0" icon-label="CHANNEL" :title="t('admin.notificationSettings.channelsAdmin.emptyTitle')" :description="t('admin.notificationSettings.channelsAdmin.emptyDescription')" />

    <section v-for="item in channels" :key="item.contract.slot" class="overflow-hidden rounded-md border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      <header class="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 dark:border-zinc-800 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div class="flex flex-wrap items-center gap-2">
            <h3 class="font-medium">{{ t(`admin.notificationSettings.channels.${channelKey(item)}`) }}</h3>
      ...
      <div class="grid gap-4 px-4 py-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <div>
    ...
    <section class="border-t border-slate-200 pt-4 dark:border-zinc-800">
      <div>
        <h3 class="font-medium">{{ t('admin.notificationSettings.channelsAdmin.deliveryTitle') }}</h3>
