...
} = await useAsyncData('admin-personalization-chrome-options', async () => {
...
} = await useAsyncData('admin-personalization-chrome-catalogs', async () => {
...
// 用 getter 暴露，避免父组件拿到 ComputedRef 对象导致 loading 恒为真。
defineExpose({
  refresh: refreshAll,
  get loading() {
    return optionsPending.value || chromePending.value
  },
  get loadError() {
    return optionsError.value || chromeError.value
  }
})
...
<template>
  <div>
    ...
    <!-- 品牌资源 -->
    <form v-if="section === 'brand'" class="flex flex-col gap-5" @submit.prevent="saveBrand">
    ...
    <!-- 导航 -->
    <div v-else-if="section === 'nav'" class="flex flex-col gap-5">
      <UCard class="border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <template #header>
          <div>
            <h3 class="text-base font-bold text-slate-900 dark:text-white">
    ...
    <!-- 公告 -->
    <div v-else-if="section === 'announcements'" class="flex flex-col gap-5">
      <UCard class="border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <template #header>
          <div>
            <h3 class="text-base font-bold text-slate-900 dark:text-white">
    ...
    <!-- 法律正文 -->
    <form v-else-if="section === 'legal'" class="flex flex-col gap-5" @submit.prevent="saveLegal">
      <UCard class="border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <template #header>
          <div>
            <h3 class="text-base font-bold text-slate-900 dark:text-white">
