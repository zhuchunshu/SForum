...
import { webReleaseCanRetry, webReleaseCanRollback, webReleaseProgress } from '~/utils/adminWebReleases'
...
<template>
  ...
  <div class="overflow-hidden border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
    <table class="w-full text-left text-sm">
      <thead class="bg-slate-50 text-xs text-slate-500 dark:bg-zinc-950"><tr><th class="px-4 py-3">ID</th><th class="px-4 py-3">{{ t('admin.extensions.releases.trigger') }}</th><th class="px-4 py-3">{{ t('admin.extensions.releases.theme') }}</th><th class="px-4 py-3">{{ t('admin.extensions.releases.status') }}</th><th class="px-4 py-3 text-right">{{ t('admin.extensions.releases.actions') }}</th></tr></thead>
      <tbody class="divide-y divide-slate-200 dark:divide-zinc-800">
        <tr v-for="release in data.items" :key="release.id">
          <td class="px-4 py-3 font-mono">#{{ release.id }}</td><td class="px-4 py-3">{{ release.triggerKind }}</td><td class="px-4 py-3">{{ release.activeThemeId }} <span class="text-slate-500">v{{ release.themeVersion }}</span></td>
          <td class="px-4 py-3"><UBadge variant="subtle">{{ release.status }}</UBadge><UProgress v-if="webReleaseProgress(release.status) < 100" class="mt-2 w-28" size="xs" :model-value="webReleaseProgress(release.status)" /></td>
          <td class="px-4 py-3"><div class="flex justify-end gap-1"><UButton size="xs" icon="i-lucide-eye" color="neutral" variant="ghost" @click="select(release.id)" /><UButton v-if="webReleaseCanRetry(release.status)" size="xs" icon="i-lucide-refresh-cw" :loading="commandId === release.id" @click="command(release.id, 'retry')" /><UButton v-if="webReleaseCanRollback(release.status)" size="xs" icon="i-lucide-undo-2" color="warning" variant="subtle" :loading="commandId === release.id" @click="command(release.id, 'rollback')" /></div></td>
        </tr>
      </tbody>
    </table>
    <div v-if="!data.items.length && !pending" class="p-10 text-center text-sm text-slate-500">{{ t('admin.extensions.releases.empty') }}</div>
  </div>
