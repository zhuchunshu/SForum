...
const states = ['', 'available', 'running', 'retryable', 'scheduled', 'completed', 'discarded', 'cancelled']
...
<template>
  <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
    ...
    <UButton icon="i-lucide-rotate-cw" color="neutral" variant="subtle" :loading="manager.jobs.pending.value" @click="manager.refresh">{{ t('admin.extensions.refresh') }}</UButton>
  </div>
  ...
  <div class="mb-3 grid gap-2 sm:grid-cols-3"><UInput v-model="manager.filters.kind" icon="i-lucide-search" :placeholder="t('admin.jobs.filterKind')" /><USelect v-model="manager.filters.state" :items="states" /><USelect v-model="manager.filters.queue" :items="['', ...manager.overview.data.value.queues.map(item => item.name)]" /></div>
