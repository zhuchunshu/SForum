...
const { legalBody, siteName } = useWebOptions()

const body = computed(() => legalBody('privacy', locale.value))

useSeoMeta({
  title: () => `${t('legal.privacyTitle')} · ${siteName.value}`
})
</script>

<template>
  <SFPageOutlet page="site.privacy">
