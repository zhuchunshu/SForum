...
describe('forum taxonomy helpers', () => {
  ...
  test('category and tag pages rely on API pagination defaults', () => {
    const categoryPage = readFileSync(new URL('../../app/components/forum/SFCategoryShowPage.vue', import.meta.url), 'utf8')
    const tagPage = readFileSync(new URL('../../app/components/forum/SFTagShowPage.vue', import.meta.url), 'utf8')
  ...
  test('topic show page loads detail via useAsyncData once (D3 no double-count path)', () => {
    const page = readFileSync(new URL('../../app/components/forum/SFTopicShowPage.vue', import.meta.url), 'utf8')
