...
describe('default theme V32 left-nav homepage contract', () => {
  ...
  test('renders topic table rows using only API-backed summary data', () => {
    ...
    expect(row).toContain('sf-home-topic-row__mobile-taxonomy')
    expect(row).toContain(':title="topic.categoryName"')
    // 桌面 C1：徽标独占一行在标题上方，分类 pill 在标题前，标签最多 3 个
    expect(row).toContain('sf-home-topic-row__badges')
    expect(row).toContain('min-[721px]:basis-full')
    expect(row).toContain('sf-home-topic-row__desktop-category')
    expect(row).toContain('desktopTags')
    expect(row).toContain('slice(0, 3)')
    expect(row).toContain('forumTagPath')
    expect(row).toContain('sf-home-topic-row__desktop-tags')
    // 移动端 G1：发帖人 ← 最近回复 · 最近活动；无回复回退 发帖人 · 发帖时间
    expect(row).toContain('hasReplies')
    expect(row).toContain('sf-home-topic-row__meta-replier')
    expect(row).toContain('sf-home-topic-row__meta-arrow')
    expect(row).toContain('i-lucide-arrow-left')
    expect(row).toContain('topic.lastActivityAt || topic.createdAt')
    expect(row).toContain('hidden min-[721px]:inline-flex')
    expect(row).toContain('hidden max-[720px]:flex')
    // 桌面不再使用旧五列表格列；移动端不再展示标签
    expect(row).not.toContain('sf-home-topic-row__category')
    expect(row).not.toContain('sf-home-topic-row__replies')
    expect(row).not.toContain('sf-home-topic-row__activity')
    expect(row).not.toContain('sf-home-topic-row__desktop-avatar')
    expect(row).not.toContain('grid-cols-[42px_minmax(0,1fr)_96px_54px_104px]')
    expect(row).not.toContain('sf-home-topic-row__mobile-replier')
    expect(row).not.toContain('sf-home-topic-row__mobile-replies')
    expect(row).not.toContain('mobileTags')
  ...
  test('registers three-column homepage stylesheet tokens with right rail', () => {
    ...
    expect(css).toContain('.sf-page.sf-theme--default .sf-home-topic-row__title')
    ...
    expect(themeHybridCss).toContain('.sf-theme--default .sf-home-topic-row__title')
    expect(themeHybridCss).toContain('overflow-wrap: anywhere;')
    expect(themeHybridCss).toContain('text-overflow: clip;')
    expect(themeHybridCss).toContain('white-space: normal;')
    ...
    expect(row).toContain('sf-home-topic-row__reply-badge')
    expect(row).toContain('sf-home-topic-row__desktop-category')
    expect(row).toContain('sf-home-topic-row__desktop-tags')
