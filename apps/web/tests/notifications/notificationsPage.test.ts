...
describe('SFNotificationsPage contract', () => {
  test('delegates the Navbar bell, unread reconciliation, and realtime preview to the notification domain', () => {
    ...
    expect(navbar).toContain("import { useNotifications } from '~/composables/notifications/useNotifications'")
    expect(navbar).toContain('const notifications = useNotifications()')
    expect(navbar).toContain('notifications.unreadCount.value')
    expect(navbar).toContain('navbar__mobile-bottom-notifications')
    const bottomNav = navbar.slice(navbar.indexOf('<nav class="navbar__mobile-bottom-nav"'))
