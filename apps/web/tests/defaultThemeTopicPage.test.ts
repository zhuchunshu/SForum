...
describe('default theme V32 topic page contract', () => {
  ...
  test('uses a full-width three-column shell with left nav and topic side card', () => {
    ...
    const sideCard = sourceFile('../app/components/SFTopicSideCard.vue')
    ...
    expect(sourceFile('../app/composables/useForumApi.ts')).toContain('listTopicContributionTimeline')
    expect(sourceFile('../app/utils/forumTaxonomy.ts')).toContain('ForumTopicContributionTimeline')
