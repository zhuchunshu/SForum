...
import { buildAuthPageLink, resolveAuthReturnPath } from '../app/utils/authReturn'
import {
...
async function compileMountedComponent(relativePath: string) {
  ...
  return new Function('Vue', executable)(mountVue)
}

const SFAuthProviderButtons = await compileMountedComponent('app/components/SFAuthProviderButtons.vue')
const SFLoginFormPage = await compileMountedComponent('app/components/SFLoginFormPage.vue')
const SFRegisterFormPage = await compileMountedComponent('app/components/SFRegisterFormPage.vue')
