...
describe('SFPageOutlet catalog wiring', () => {
  ...
  it('routes 404 errors through the dedicated themed not-found boundary', () => {
    ...
    expect(errorPage).toContain('serverInternal: import.meta.server')
    expect(errorPage).toContain('serverInternal: true')
    expect(errorPage).toContain("if (resolved.provider === 'core' || resolved.fallback)")
    expect(errorPage).toContain('exactThemeIdentityForPageResolve(resolved)')
    expect(errorPage).toContain('enterCoreEmergency(')
    expect(errorPage).toContain('Promise.all([')
    expect(errorPage).not.toContain('Promise.allSettled')
    ...
    expect(errorPage).toContain('apply: false')
    expect(errorPage).toContain('themeSkin.commit(skinResult)')
