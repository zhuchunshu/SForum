...
function getCaptchaInputValue() {
  var inputs = document.querySelectorAll('input[isCaptchaInput]');
  var v;
  if (inputs.length > 0) {
    v = inputs[0].value;
  }
  ...
  if (document.getElementById("captcha-container")) {
    turnstile.render('#captcha-container', {
      ...
      callback: function callback(token) {
        ...
        var captchaInputs = document.querySelectorAll('input[isCaptchaInput]');
        captchaInputs.forEach(function (input) {
          input.value = token;
        });
        localStorage.setItem("captcha_token", token);
        var needCaptchaBtn = document.querySelectorAll('button[isNeedCaptcha]');
        needCaptchaBtn.forEach(function (button) {
...
window.onloadGoogleRecaptchaCallback = function () {
  if (document.getElementById("captcha-container")) {
    grecaptcha.render('captcha-container', {
      'sitekey': captcha_config.recaptcha,
      //公钥
      'theme': system_theme,
...
document.addEventListener("DOMContentLoaded", function () {
  var buttons = document.querySelectorAll("button[isNeedCaptcha]");
