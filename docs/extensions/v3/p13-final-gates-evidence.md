...
| Browser desktop/mobile + JS-disabled + Baiduspider | green with residual honesty | Live Nuxt `:3000` Baiduspider `/` `/login` `/register` 200 with titles; home includes JSON-LD. Unit: `SEORegistry/product_js_disabled_test.go`. Mobile viewport: `apps/web/tests/p9JoinedVisualMatrix.test.ts`. L2 fail-closed/quarantine: `Models/Extensions/public_l2_p9_joined_matrix_test.go`, `apps/web/tests/publicExtensionQuarantine.test.ts`. Historical Playwright: P8 crawler session. Residual honesty: full opt-in Playwright multi-viewport matrix is env-gated (`SFORUM_PUBLIC_L2_E2E=1`), not a default CI suite. |
| Legacy deletion | protocol complete; theme loader blocked | Manifest V3 / Protocol V2 are exclusive. The request-time theme loader retains its independent APILTS gate. |
| Product-path residual (2026-07-21) | closed | Media MIME on avatar/SEO; public content-catalog; entity import/export dry-run allow/deny; i18n for dry-run error codes (`a155ed44d`…`fix(i18n) entity dry-run`). |
| Full `go test ./...` re-run after product-path | re-verified green | Localization `entity.*` i18n; catalog inventory **249**; entity dry-run `extension.view` handler + `core.guard.extensions.read` partition; full suite EXIT 0 after guard fix. |
| APILTS inspect | protocol entry removed | `go run ./cmd/sforum extension api-lts --json` exposes only current Manifest V3 / Protocol V2 plus independently governed compatibility surfaces such as the theme loader. |
