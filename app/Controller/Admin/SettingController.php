 ...
 */
#[Controller]
#[Middleware(middleware: 'App\\Middleware\\AdminMiddleware')]
class SettingController
{
    ...
    {
        return $ui->title("个人设置")->ImportJs([mix("js/admin/setting.js")])->body($card->title("个人设置")->content(view("admin.setting.im"))->render())->render();
    }
    /**
     * 个人设置
     */
    ...
    public function imPost() : array
    {
        ...
        switch ($type) {
            case 'username':
                ...
                AdminUser::query()->where("id", Admin::id())->update(["username" => request()->input("username")]);
                $data = AdminUser::query()->where('id', Admin::id())->first();
                session()->set('admin', $data);
                return Json_Api(200, true, ["msg" => "修改成功!"]);
            case 'email':
                $v = $this->validationFactory->make(request()->all(), ['email' => 'required|email'])->validated();
                AdminUser::query()->where("id", Admin::id())->update(["email" => $v['email']]);
                $data = AdminUser::query()->where('id', Admin::id())->first();
                session()->set('admin', $data);
                return Json_Api(200, true, ["msg" => "修改成功!"]);
                break;
            case 'password':
                $v = $this->validationFactory->make(request()->all(), ['old_pwd' => 'required', 'new_pwd' => 'required|min:6'], [], ['old_pwd' => "旧密码", 'new_pwd' => "新密码"])->validated();
                if (!Hash::check($v['old_pwd'], AdminUser::query()->where("id", Admin::id())->first()->password)) {
                    return Json_Api(403, false, ["旧密码错误"]);
                }
                AdminUser::query()->where("id", Admin::id())->update(["password" => Hash::make($v['new_pwd'])]);
                $data = AdminUser::query()->where('id', Admin::id())->first();
                session()->set('admin', $data);
                return Json_Api(200, true, ["msg" => "修改成功!"]);
                break;
    ...
    #[PostMapping("/admin/setting/clearCache")]
    ...
    #[PostMapping('/api/adminOptionList')]
