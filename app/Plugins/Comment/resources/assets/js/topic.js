...
"use strict";
/*!***********************************************!*\
  !*** ./resources/js/plugins/Comment/topic.js ***!
  \***********************************************/
...
$(function () {
  $('a[comment-click="comment-like-topic"]').click(function () {
    ...
    axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/comment/like.topic.comment", {
      ...
      if (!data.success) {
        ...
        var likes_text = $(_this).children('span[comment-show="comment-topic-likes"]');
        var y_likes = likes_text.text();
        y_likes = parseInt(y_likes);

        if (data.code === 200) {
          $(_this).children('span[comment-show="comment-topic-likes"]').text(y_likes + 1);
        } else {
          $(_this).children('span[comment-show="comment-topic-likes"]').text(y_likes - 1);
        }
      }
...
$(function () {
  $("#reply-comment-modal-reply-button").bind("click", function () {
    var comment_id = $("#reply-comment-id").val();

    if (!comment_id) {
      swal('Error', '回复评论的ID不存在，请刷新页面重试', 'error');
    }

    var content = $("#reply-comment-content").val();

    if (!content) {
      swal('Error', '回复内容为空!', 'error');
    }

    axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/comment/comment.topic.reply", {
      content: content,
      comment_id: comment_id,
      _token: csrf_token
    }).then(function (r) {
...
$(function () {
  $('a[comment-click="comment-delete-topic"]').click(function () {
    ...
    swal({
      ...
      if (willDelete) {
        axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/comment/comment.topic.delete", {
          _token: csrf_token,
          comment_id: comment_id
        }).then(function (r) {
          var data = r.data;
...
$(function () {
  $('a[comment-click="comment-caina-topic"]').click(function () {
    var th = $(this);
    var comment_id = th.attr("comment-id");
    axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/comment/topic.caina.comment", {
      _token: csrf_token,
      comment_id: comment_id
    }).then(function (r) {
      var data = r.data;
...
$(function () {
  $('a[comment-click="star-comment"]').click(function () {
    ...
    var comment_id = th.attr("comment-id");
    axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/comment/star.comment", {
      _token: csrf_token,
      topic_id: topic_id,
      comment_id: comment_id
    }).then(function (r) {
