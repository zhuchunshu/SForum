...
class EditMiddleware implements MiddlewareInterface
{
    ...
    {
        $validator = $this->validationFactory->make($data, ['comment_id' => 'required|exists:topic_comment,id', 'content' => 'required|string|min:' . get_options('comment_create_min', 1) . '|max:' . get_options('comment_create_max', 200)], [], ['comment_id' => '评论ID', 'content' => '评论内容']);
        if ($validator->fails()) {
            // Handle exception
        ...
        if (get_options('comment_change_limit') === 'true' && time() - \Carbon\Carbon::parse(@$comment->created_at)->timestamp > (int) get_options('comment_change_limit_time', 5) * 60) {
            return redirect()->back()->with('danger', ' 评论发布时间已超过' . get_options('comment_change_limit_time', 5) . '分钟，禁止修改!')->go();
        }
