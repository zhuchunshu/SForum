...
{
    // 回复评论
    #[PostMapping('comment.topic.reply')]
    public function topic_reply_create(TopicReply $request) : bool|array
    {
        ...
        $post = Post::query()->create(['content' => $content, 'user_agent' => get_user_agent(), 'user_ip' => get_client_ip(), 'user_id' => auth()->id()]);
        $data = TopicComment::query()->create(['post_id' => $post->id, 'parent_url' => '/' . $topic_id . '.html/' . $comment_id . '?page=' . get_topic_comment_page((int) $comment_id), 'topic_id' => $topic_id, 'parent_id' => $comment_id, 'user_id' => auth()->id()]);
    ...
    // 删除帖子评论
    #[PostMapping('comment.topic.delete')]
    ...
    public function topic_comment_list()
    {
        ...
        $page = TopicComment::query()->where(['topic_id' => $topic_id])->with('topic', 'user')->paginate((int) get_options('comment_page_count', 2));
    ...
    public function like_topic_comment() : array
    {
        ...
        TopicCommentLike::query()->create(['comment_id' => $comment_id, 'user_id' => auth()->id()]);
        return Json_Api(200, true, ['msg' => '已赞!']);
    }
    #[PostMapping('topic.comment.data')]
    public function topic_comment_data() : array
    {
        ...
        $data = TopicComment::query()->where([['id', $comment_id]])->first();
    ...
    public function topic_caina_comment()
    {
        ...
        $data = TopicComment::query()->where([['id', $comment_id]])->with('topic')->first();
        $quanxian = false;
        ...
        if ($data->optimal === null) {
            TopicComment::query()->where([['id', $comment_id]])->update(['optimal' => date('Y-m-d H:i:s')]);
            $caina = __('topic.comment.adoption');
        } else {
            TopicComment::query()->where([['id', $comment_id]])->update(['optimal' => null]);
        }
    ...
    public function star_topic() : array
    {
        ...
        UsersCollection::query()->create(['user_id' => auth()->id(), 'type' => 'comment', 'type_id' => $comment_id]);
        return Json_Api(200, true, ['msg' => '已收藏']);
    }
    // 获取评论者IP
    #[PostMapping('get.user.ip')]
    #[RateLimit(create: 1, capacity: 3)]
    public function get_user_ip()
    {
        ...
        foreach ($comments as $comment_id) {
            ...
            if ($comment->post->user_ip) {
                $data[] = ['comment_id' => $comment->id, 'text' => __('app.IP attribution', ['province' => get_client_ip_data($comment->post->user_ip)['pro']])];
            }
        }
        return Json_Api(200, true, $data);
    }
            ...
            go(function () use($username, $data) {
                if (User::query()->where('username', $username)->exists()) {
                    ...
                    if ((int) $user->id !== (int) $data->user_id) {
                        user_notice()->send($user->id, '有人在评论中提到了你', '有人在评论中提到了你', get_topic_comment_url($data->id));
                    }
                }
            });
        }
