...
class UserClass
{
    ...
    public function create_post(Create $request) : array
    {
        ...
        $result = ["icon" => $data['icon'], "name" => $data['name'], "color" => $data['color'], "quanxian" => json_encode($data['quanxian'], JSON_UNESCAPED_UNICODE), 'permission-value' => $data['permission-value']];
        \App\Plugins\User\src\Models\UserClass::query()->create($result);
        return Json_Api(200, true, ['msg' => '用户组创建成功!']);
    }
    #[GetMapping('/admin/userClass/edit/{id}')]
    ...
    #[PostMapping('/admin/userClass/{id}/data')]
    public function data($id)
    {
        ...
        $result = ["icon" => $data->icon, "name" => $data->name, "color" => $data->color, "quanxian" => json_decode($data->quanxian), 'permission_value' => $data['permission-value']];
        return Json_Api(200, true, $result);
    }
    #[PostMapping('/admin/userClass/update')]
    public function update(Update $request) : array
    {
        ...
        $data = $request->validated();
        \App\Plugins\User\src\Models\UserClass::query()->where("id", $data['id'])->update(["icon" => $data['icon'], "name" => $data['name'], "color" => $data['color'], "quanxian" => json_encode($data['quanxian'], JSON_UNESCAPED_UNICODE), 'permission-value' => $data['permission-value']]);
        return Json_Api(200, true, ['msg' => '更新成功!']);
    }
    #[PostMapping("/admin/userClass/remove")]
    ...
    #[PostMapping("/admin/userClass/Default/Authority")]
