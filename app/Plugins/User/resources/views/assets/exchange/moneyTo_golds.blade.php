...
<div class="mb-2">
    ...
    @if(round($user->Options->money - (intval($user->Options->money*get_options('wealth_how_many_money_to_golds','1')))/get_options('wealth_how_many_money_to_golds','1'),$dc)!=0)
        有 {{round($user->Options->money - (intval($user->Options->money*get_options('wealth_how_many_money_to_golds','1')))/get_options('wealth_how_many_money_to_golds','1'),$dc)}} {{get_options('wealth_money_unit_name', '元')}} 不能兑换
    @endif
</div>
...
<div class="mb-2">
    ...
    <input type="number" min="0" step="1" max="{{intval($user->Options->money*get_options('wealth_how_many_money_to_golds','1'))}}" class="form-control" v-model="data.moneyTo_golds_num" placeholder="输入兑换的{{get_options('wealth_golds_name', '金币')}}数量" required>
</div>