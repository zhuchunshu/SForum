...
<div class="row row-cards">
    ...
    <div class="col-md-12">
        <div class="border-0 card card-body">
            <h3 class="card-title">{{$user->username}} 关注的人</h3>
            @if($users->count())
                <div class="row row-cards">
                        ...
                        <div class="col-md-6 col-lg-4">
                            <div class="card">
                                <div class="card-body p-4 text-center">
                                    <span class="avatar avatar-xl mb-3 avatar-rounded" style="background-image: url({{super_avatar($value->user)}})"></span>
                                    <h3 class="m-0 mb-1"><a href="#">{{$value->user->username}}</a></h3>
{{--                                    <div class="text-muted">UI Designer</div>--}}
                                    <div class="mt-3">
                                        <a href="/users/group/{{$value->user->class_id}}.html" class="badge badge-outline"
                                           style="color: {{$user->Class->color}}">{{$value->user->Class->name}}</a>
                                    </div>
                                </div>
                                <div class="d-flex">
                                    ...
                                    @if(auth()->id()!==(int)$value->user->id)
                                        <a href="/users/pm/{{$value->user->id}}" class="card-btn"><!-- Download SVG icon from http://tabler-icons.io/i/phone -->
