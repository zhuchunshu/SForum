...
class TagsController
{
    ...
    public function data($id)
    {
        ...
        $page = Topic::query(true)->where('tag_id', $id)->with('tag', 'user')->orderBy('topping', 'desc')->orderByDesc('last_time')->orderBy('updated_at', 'desc')->paginate((int) get_options('topic_home_num', 15));
        if (request()->input('query') === 'hot') {
            $page = Topic::query()->where('tag_id', $id)->with('tag', 'user')->orderBy('view', 'desc')->orderBy('id', 'desc')->paginate((int) get_options('topic_home_num', 15));
        }
        if (request()->input('query') === 'publish') {
            $page = Topic::query()->where('tag_id', $id)->with('tag', 'user')->orderBy('id', 'desc')->paginate((int) get_options('topic_home_num', 15));
        }
        if (request()->input('query') === 'essence') {
            $page = Topic::query()->where('tag_id', $id)->where([['essence', '>', 0]])->with('tag', 'user')->orderByDesc('last_time')->orderBy('updated_at', 'desc')->paginate((int) get_options('topic_home_num', 15));
        }
        if (request()->input('query') === 'topping') {
            $page = Topic::query()->where('tag_id', $id)->where([['topping', '>', 0]])->with('tag', 'user')->orderByDesc('last_time')->orderBy('updated_at', 'desc')->paginate((int) get_options('topic_home_num', 15));
        }
   ...
   <line x1="8" y1="16" x2="12" y2="16"></line>
    ...
    public function create_store(CreateTagRequest $request)
    {
        ...
        TopicTag::create(['name' => $name, 'color' => $color, 'description' => $description, 'icon' => $icon, 'userClass' => $userClass, 'status' => $create_status, 'user_id' => auth()->id()]);
        if (get_options('topic_create_tag_ex', 'false') === 'true') {
...
HTML;
    ...
    public function edit_post(EditTagRequest $request, AvatarUpload $upload)
    {
        ...
        TopicTag::query()->where('id', $id)->update(['name' => $name, 'description' => $description, 'color' => $color, 'icon' => request()->input('icon'), 'userClass' => $userClass]);
