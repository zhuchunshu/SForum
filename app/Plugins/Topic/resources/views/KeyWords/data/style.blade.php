    ...
    <div class="d-flex border-0 card">
        <div class="row">
            <div class="col-auto align-self-center">
                <a href="/users/{{$data->topic->user->id}}.html" class="avatar"
                   style="background-image: url({{super_avatar($data->topic->user)}});--tblr-avatar-size: 2.8rem;">
                </a>
                                ...
                                <h3 class="text-muted @if(get_options('theme_home_title_truncate')==="true"){{"text-truncate"}}@endif">
                                    ...
                                    @else
                                        <span class="badge d-none d-lg-inline-block"
                                              style="background-color: {{$data->topic->tag->color}}!important;">
                                                        {{$data->topic->tag->name}}
                                                    </span>
                    ...
                    <div class="col-md-12">
                        <div class="d-flex align-items-center">
                            <div class="text-muted-sm" style="margin-top:5px">
                                {!! u_username($data->topic->user,['class' => ['text-muted-sm']]) !!} {{format_date($data->topic->created_at)}}

                                @if($data->topic->comments->count())
                                    ←
                                    {{--                                最后回复--}}
                                    {!! u_username($data->topic->comments->last()->user,['class' => ['text-muted-sm']]) !!}  {{format_date($data->topic->comments->last()->created_at)}}

                                    {{--                                    @if($data->topic->updated_at>$data->topic->comments->last()->created_at)--}}
                                    {{--                                        {!! u_username($data->topic->user,['class' => ['text-muted-sm']]) !!} {{format_date($data->topic->updated_at)}}--}}
                                    {{--                                    @else--}}
                                    {{--                                        {!! u_username($data->topic->comments->last()->user,['class' => ['text-muted-sm']]) !!}  {{format_date($data->topic->comments->last()->created_at)}}--}}
                                    {{--                                    @endif--}}
                            ...
                            <div class="ms-auto d-none d-lg-inline-block">
                                ...
                                <a style="text-decoration:none;" href="/{{$data->topic->id}}.html#topic-comment"
                                   ...
                                   data-bs-placement="bottom" title="{{__("app.comment")}}">
                                    ...
                                    <span core-show="topic-likes">{{$data->topic->comments->count()}}</span>
                                </a>
                            </div>
                        </div>
                    </div>
