...
"use strict";
/*!*********************************************!*\
  !*** ./resources/js/plugins/Core/report.js ***!
  \*********************************************/
  ...
  var rdcf = {
    ...
    methods: {
      submit: function submit() {
        ...
        axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/core/report/update", {
          _token: csrf_token,
          report_id: report_id
        }).then(function (r) {
          if (!r.data.success) {
      ...
      remove: function remove() {
        sweetalert__WEBPACK_IMPORTED_MODULE_2___default()("确定要删除此举报吗? 删除后不可恢复", {
          dangerMode: true,
          buttons: true
        }).then(function (value) {
          if (value === true) {
            axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/core/report/remove", {
              _token: csrf_token,
              report_id: report_id
            }).then(function (r) {
              if (!r.data.success) {
      ...
      init: function init() {
        ...
        axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/core/report/data", {
          _token: csrf_token,
          report_id: report_id
        }).then(function (r) {
          if (!r.data.success) {
            ...
            if (result.status === "pending") {
              _this2.btn["class"].isPrimary = true;
              _this2.btn["class"].isDanger = false;
              _this2.btn.text = "批准";
            }

            if (result.status === "approve") {
              _this2.btn["class"].isPrimary = false;
              _this2.btn["class"].isDanger = true;
              _this2.btn.text = "驳回";
            }
