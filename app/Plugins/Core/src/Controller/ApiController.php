...
**举报原因:** ' . $request->input('report_reason') . "\n\n" . $request->input('content');
        $data = Report::query()->create(['post_id' => $post_id, 'user_id' => auth()->id(), 'title' => $request->input('title'), 'content' => xss()->clean(markdown()->text($content))]);
        // 发送通知
        $users = [];
    ...
    // 获取所有被举报并批准的的评论
    #[PostMapping('report/approve.comment')]
    ...
    public function report_update()
    {
        ...
        Report::query()->where('id', $report_id)->update(['status' => $_status]);
        if ($_status === 'approve') {
            $post_status = 'report';
        } else {
            $post_status = 'publish';
        ...
        go(function () use($report_id, $post_status) {
            ...
            if ($post->comment_id) {
                // 评论
                \Hyperf\DbConnection\Db::table('topic_comment')->where('id', $post->comment_id)->update(['status' => $post_status]);
            } else {
                if ($post->topic_id) {
                    // 主题
                    \Hyperf\DbConnection\Db::table('topic')->where('id', $post->topic_id)->update(['status' => $post_status]);
                }
            }
        });
    ...
    public function report_remove()
    {
        ...
        go(function () use($report_id) {
            ...
            if ($post->comment_id) {
                // 评论
                \Hyperf\DbConnection\Db::table('topic_comment')->where('id', $post->comment_id)->update(['status' => 'publish']);
            } else {
                if ($post->topic_id) {
                    // 主题
                    \Hyperf\DbConnection\Db::table('topic')->where('id', $post->topic_id)->update(['status' => 'publish']);
                }
            }
        });
        // 发送通知
    ...
    #[RequestMapping('useragentinfo')]
    public function useragentinfo()
    {
        return ['useragent' => get_user_agent(), 'ip' => get_client_ip(), 'ip_info' => get_client_ip_data(get_client_ip())];
    }
}