...
class SFPayController
{
    ...
    public function paying_submit($order_id)
    {
        ...
        // 扣款
        if (!PayAmountRecord::query()->where(['original' => $money, 'cash' => $money - $order->amount, 'user_id' => auth()->id(), 'order_id' => $order->id, 'remark' => '购买:【' . $order->title . '】'])->exists()) {
