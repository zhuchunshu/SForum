...
class FriendLinksController
{
    ...
    {
        $validator = $this->validationFactory->make(request()->all(), ['name' => 'required', 'link' => 'required', 'icon' => 'nullable', 'to_sort' => 'required|integer', '_blank' => 'required', 'hidden' => 'required', 'description' => 'nullable|string'], [], ['name' => '友链名称', 'link' => '链接', 'icon' => '图标', 'to_sort' => '排序', '_blank' => '是否新标签打开', 'hidden' => '是否在首页隐藏', 'description' => '网站描述']);
        if ($validator->fails()) {
            // Handle exception
