...
class UserController
{
    ...
    public function forgot_password_submit(ForgotPassword $request)
    {
        ...
        User::query()->where('email', $email)->update(['password' => Hash::make($data['password'])]);
...
HTML;
    ...
    public function register_post(RegisterRequest $request): array
    {
        ...
        $result = User::query()->create(['username' => $username, 'email' => $email, 'password' => Hash::make($password), 'class_id' => get_options('core_user_reg_defuc', 1), '_token' => Str::random(), 'options_id' => $userOption->id]);
        // 设置邀请码失效
        InvitationCode::query()->where(['code' => $invitationCode, 'status' => false])->update(['user_id' => $result->id, 'status' => true]);
