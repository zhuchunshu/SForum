...
class AliPay implements PayInterFace
{
    ...
    private array $pay_status = ['WAIT_BUYER_PAY' => '交易创建', 'TRADE_FINISHED' => '交易完成', 'TRADE_CLOSED' => '交易关闭', 'TRADE_SUCCESS' => '支付成功'];
    /**
     * 支付配置.
    ...
    public function create(PayOrder $order) : mixed
    {
        ...
        if (get_options('alipay_pay_mode', 'MIX') === 'SCAN') {
            $create_order = ['out_trade_no' => (string) $order->id, 'subject' => $order->title, 'total_amount' => $this->calculate_amount($order->amount), 'quit_url' => url()];
    ...
    public function notify($request) : bool|array|\Psr\Http\Message\ResponseInterface
    {
        ...
        $notify_result = pay()->notify($result['out_trade_no'], $this->pay_status[$result['trade_status']], $result['trade_no'], $this->calculate_amount($result['receipt_amount'], true), $result, $this->calculate_amount($result['total_amount'], true), '[2,"aliPay"]');
    ...
    {
        $result = $this->pay()->find(['out_trade_no' => $order->id, 'trade_no' => $order->trade_no]);
        return [
            'amount' => $order->amount,
            //预收金额
            'payer_total' => $order->payer_total,
            //实收金额
            'status' => $this->pay_status[$result->trade_status],
            // 订单状态
            'amount_total' => $result->total_amount,
            // 订单总金额
            'success_time' => @$result->send_pay_date ?: null,
        ];
    }
    /**
     * 关闭订单.
