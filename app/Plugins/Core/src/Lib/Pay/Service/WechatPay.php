...
class WechatPay
{
    ...
    public function find(PayOrder $order): array
    {
        ...
        return [
            ...
            'payer_total' => $this->calculate_amount($result->amount['payer_total'], true), //实收金额
            'status' => $result->trade_state_desc, // 订单状态
            'amount_total' => $this->calculate_amount($result->amount['total'], true), // 订单总金额
            'success_time' => @$result->success_time ?: null // 支付完成时间
        ];
    }
