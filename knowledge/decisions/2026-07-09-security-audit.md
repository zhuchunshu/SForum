...
### [ ] C1. 扩展 ZIP 通过 `manifest.Version` 路径穿越 → 任意文件写入 / RCE
...
### [ ] H1. `user.manage` 权限可自我提权到 super_admin
...
### [ ] H2. FTP/SFTP 凭证明文存储 + SFTP 主机密钥默认不校验（MITM）
...
### [ ] H3. 评论/主题正文 `v-html` 单点失败（无前端二次净化）
...
### [ ] M4. `/auth/registration-status` 公开暴露系统初始化状态
...
### [ ] M6. 管理员搜索/DB 浏览器 LIKE 未转义 + 深分页无 page 上限（DoS）
...
### [ ] M7. 个人资料 `websiteUrl` 经 `:href` 绑定 → `javascript:` XSS
...
### [ ] M9. session `Secure` 依赖 `APP_ENV` 字符串
