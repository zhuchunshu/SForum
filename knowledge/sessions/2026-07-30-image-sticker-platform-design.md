...
- Landed the Forum Canvas base surface in production `SFEditor`: extracted a
  ...
  retained write/preview switching, and applied the quiet desktop/mobile
  document geometry to full, compact, and basic-field presets.
- Removed the visible Markdown source and native JSON inspection modes after
  operator confirmation. Internal Markdown and native Tiptap JSON payloads are
  still emitted through the existing editor persistence contract.
...
- Production Browser QA passed at `http://127.0.0.1:3000/topics/new` in the
  ...
  bold plus write/preview transitions, and no console warnings/errors.
- Follow-up Browser QA at `1280x720` and `390x844` confirmed exactly two view
  controls (`撰写`, `预览`), working preview content, internal mobile toolbar
  scrolling, and no horizontal page overflow or application console errors.
...
- The production editor exposes only write and preview modes. Markdown and
  native JSON remain internal persistence outputs rather than user-facing
  inspection surfaces.
