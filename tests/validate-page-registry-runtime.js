...
function validateOfflineContracts() {
  ...
  assertNotIncludes(template, 'import(/* @vite-ignore */', 'SFThemeTemplate must not import package code directly')
  assertIncludes(template, "'core.component.shared.sfextension_widget': defineAsyncComponent(() => import('./SFExtensionWidget.vue'))", 'trusted L2 must use the reviewed Host island')
  ...
  assertIncludes(errorPage, 'systemErrorPageIdForStatus', 'error boundary must select the virtual Page Registry id from status')
  assertIncludes(errorPage, 'useSystemErrorPagePresentation(systemPageId)', 'system errors must pre-resolve selected-theme presentation')
  assertIncludes(errorUtils, "return content(status, 'system.forbidden'", '403 maps to system.forbidden')
  assertIncludes(errorUtils, "return content(status, 'system.not_found'", '404 maps to system.not_found')
  assertIncludes(errorUtils, "return content(status, 'system.rate_limited'", '429 maps to system.rate_limited')
  assertIncludes(errorUtils, "return content(status, 'system.server_error'", '5xx maps to system.server_error')
  assertIncludes(systemTemplate, "'system.component.error_details': SFSystemErrorDetails", 'system error details must stay a Host island')
  assertIncludes(systemTemplate, "'system.component.error_actions': SFSystemErrorActions", 'system error actions must stay a Host island')
  assertNotIncludes(systemTemplate, "'core.component.shared.sfextension_widget'", 'system error templates must not admit public L2 widgets')
